...
let height = 768;
let segments = [];
let roadWidth = 3000;                    // actually half the roads width, easier math if the road spans from -roadWidth to +roadWidth
let segmentLength = 300;                     // length of a single segment
...
function renderSegment(ctx, width, lanes, x1, y1, w1, x2, y2, w2, color) {
    ...
    var r1 = rumbleWidth(w1, lanes) * 2,
        r2 = rumbleWidth(w2, lanes) * 2,
        l1 = laneMarkerWidth(w1, lanes) * 2,
        l2 = laneMarkerWidth(w2, lanes) * 2,
        lanew1, lanew2, lanex1, lanex2, lane;


        if (i < 10) console.log(width, lanes, x1, y1, w1, x2, y2, w2, color); i++;
    ...
    // // ctx.drawImage(img, x, y, canvas, height);

    // ctx.drawImage(img, 0, 0, canvas.width / 1.5, canvas.height / 1.5);
